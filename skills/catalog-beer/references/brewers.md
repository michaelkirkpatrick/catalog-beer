...
**A domain may be shared; a homepage may not.** The catalog holds one brewer per *homepage*, not per domain: a domain's root URL belongs to at most one brewer, no two brewers may store the same page, and any number of brewers may live on distinct sub-pages of one domain. That is how an acquired brand that still sells under its own name is recorded — its `url` is its page on the owner's site (`https://northchair.com/whetstone-beer/`), beside the owner's own record at the root. Never store the owner's homepage on the acquired brand. A write that collides — a second brewer at a homepage another holds, or at a page another holds — is refused with `valid_msg.url` naming the brewer that holds it; the fix is the right page, not a different site. Because staff status comes from the email domain, an address at the owner's domain is staff on every record that stores it.
