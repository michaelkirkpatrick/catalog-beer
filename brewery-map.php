...
<body>
    ...
    <div class="container-fluid">
        <?php
            ...
            <script>
            function initMap() {
                // JSON_HEX_TAG is the load-bearing flag: these carry raw brewer
                // and location names, and inside an inline script the HTML
                // tokenizer is looking for "<", not for quotes. Default
                // json_encode escapes "/", so a closing script tag on its own
                // already fails to break out — but an HTML comment opener
                // followed by an opening script tag puts the tokenizer into the
                // script-data-double-escaped state, where the real terminator
                // stops ending the element and the rest of the page is swallowed.
                //
                // Never write either sequence literally in here. The tokenizer
                // does not know it is inside a JS comment, so a literal closing
                // script tag ends the element, leaves initMap() unterminated, and
                // blanks the map. This comment did exactly that on staging until
                // 8 Aug 2026. See classes/helpers/html.php.
